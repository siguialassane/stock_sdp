...
export function TypeForm({ target, onClose }: { target?: CatalogEditTarget; onClose: () => void }) {
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    try {
      if (editing) {
        await store.updateType(editing.id, name.trim(), normalizedCode);
        toast.success({ title: "Type modifie", message: `${name.trim()} a ete mis a jour.` });
      } else {
        await store.addType(name.trim(), normalizedCode);
        toast.success({ title: "Type cree", message: `${name.trim()} (${normalizedCode}) a ete ajoute au catalogue.` });
      }
      onClose();
    } catch (reason) {
